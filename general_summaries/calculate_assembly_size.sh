...
###  f it i've come this far let's look at gc content mferss

  read A C G T N <<< $(/programs/kentUtils/bin/faCount ../genomes/$species.fasta | tail -1 | cut -f 3,4,5,6,7 )
...
#  printf '%s\t%s\t%s\t%s\t%s\n' "$species" "$genome" "$gNA" "$tebp" "$tNA"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$species" "$six" "$genome" "$A" "$C" "$G" "$T" "$N" #"$tebp"
