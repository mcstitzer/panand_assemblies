...
chromosomes <- unique(genomecount$seqnames[genomecount$width>10e6])
...
all_seqs_to_plot=chromosomes

pdf(paste0('~/transfer/chromosomes_', genotype, '.pdf'), 20,40)
...
#for (chromosome in all_seqs_to_plot) {
  ...
  longest_filtered <- genomecount[genomecount$seqnames %in%all_seqs_to_plot, ]
  genes_filtered <- genes[genes$seqnames%in%all_seqs_to_plot, ]
  ...
  if (nrow(longest_filtered) == 0 | nrow(genes_filtered) == 0) {
    ...
    boundaries_filtered <- intermediate_boundaries[intermediate_boundaries$seqname %in%all_seqs_to_plot, ]
