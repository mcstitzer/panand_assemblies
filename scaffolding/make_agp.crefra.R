...
fai <- read_delim("fais/Cr-AUB069-DRAFT-PanAnd-1.0.fasta.fai", delim = "\t", col_names = FALSE)
...
## also need to flip an inversion in crefra - 
filtered_data$freqStrand[filtered_data$queryChr=='scaf_14']=ifelse(filtered_data$freqStrand[filtered_data$queryChr=='scaf_14']=='+', '-','+')
...
## add chromosome names by length
agp_df <- agp_df %>%
  group_by(object) %>%
...
# Save AGP file
write_delim(final_agp_df, "crefra.agp", delim = "\t", col_names = FALSE)
